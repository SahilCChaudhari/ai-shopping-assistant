...
#     def _handle_mock_checkout():
#         """Simulate payment for automated testing"""
#         cart_items = CartItem.query.all()
#         if not cart_items:
#             return jsonify({"message": "Cart is empty"}), 400
...
#         if not cart_items:
#             return jsonify({"message": "Cart is empty"}), 400
#          # ✅ Set Stripe key RIGHT BEFORE making API call (and only if mock mode is OFF)
#         if not current_app.config.get("STRIPE_SECRET_KEY"):
#             return jsonify({"error": "Stripe secret key not configured"}), 500
#         stripe.api_key = current_app.config["STRIPE_SECRET_KEY"]  # ✅ Set here, not at app startup
...
#     return app  # ← Must be at the end of create_app(), properly indented
