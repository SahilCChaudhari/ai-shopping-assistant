...
function renderOrderStatus(data) {
  ...
  const itemsHtml = (data.items && data.items.length > 0)
    ? `
      <div class="order-items">
        <p style="font-weight:600;color:var(--ink);margin-bottom:.75rem;">Items Ordered</p>
        ${data.items.map(item => `
          <div class="order-item-row">
            <div>
              <p style="font-weight:500;color:var(--ink);font-size:.9rem;">${item.product_name || item.name || "Product"}</p>
              <p style="font-size:.8rem;color:var(--ink-3);">Qty: ${item.quantity}</p>
            </div>
            <p style="font-weight:600;color:var(--ink-2);font-size:.9rem;">$${(item.price * item.quantity).toFixed(2)}</p>
          </div>
        `).join("")}
      </div>
    `
  ...
  statusDiv.innerHTML = `
    <div class="status-card">
      ...
      <p><b>Total:</b> $${parseFloat(data.total_amount).toFixed(2)}</p>
...
async function checkout() {
  ...
  try {
    ...
    if (data.mode === "mock") {
      ...
      document.getElementById("loaderText").textContent = "Redirecting to Stripe…";
...
window.onload = function () {
  ...
  const sessionId = params.get("order_id");
