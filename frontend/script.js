...
async function searchProducts() {
  ...
  try {
    ...
    resultsDiv.innerHTML = `<p>Server error. Is Flask running?</p>`;
  }
}
...
async function addToCart(productId, btn) {
  try {
    ...
    // ✅ Show feedback on button, stay on products tab
...
async function viewCart() {
  ...
  try {
    ...
    // ✅ Add total + checkout button at the bottom
...
async function removeFromCart(productId) {
  try {
    ...
    // ✅ Refresh cart in place, stay on cart tab
    await viewCart();
...
async function checkout() {
  try {
    const response = await fetch(`${BASE_URL}/checkout`, { method: "POST" });
    const data = await response.json();
